...
export const shiftAssign = functions.firestore
  ...
  .onUpdate(async (snap) => {
    try {
      ...
      const {ShiftDate, ShiftStartTime, ShiftEndTime, ShiftLocationAddress, ShiftName, ShiftAssignedUserId, ShiftCompanyId} = shiftNewData;


      if (oldAssignedUsers.length !== ShiftAssignedUserId.length || !oldAssignedUsers.every((value, index) => value === ShiftAssignedUserId[index])) {
      //* Fetch company
        const cmpSnapshot = await securityAppAdmin.firestore()
          .doc(`${CollectionName.companies}/${ShiftCompanyId}`).get();
        ...
        if (ShiftAssignedUserId && ShiftAssignedUserId.length > 0) {
          await Promise.all(
            ShiftAssignedUserId.map(async (id)=>{
              const empSnapshot = await securityAppAdmin.firestore()
              ...
              if (empData) {
                ...
                if (empEmail) {
                  await axios.post("https://backend-sceurity-app.onrender.com/api/send_email", {
